...
def reconcile(g: Graph, res: URIRef, args: AttribDict):
    ...
    #if there are multiple target products of the same type, delete any 'stub' ones that have no url, as these stub ones were created by codemetapy based on interface type and carry the same name as the SoftwareSourceCode resource. The stub entries are generated by guess_interfacetype(), called by enrich()
